...
function EditMatchPage({ role, setRole }) {
  ...
  // Load existing match data when page opens
  useEffect(function () {
    async function fetchMatch() {
      try {
        ...
        setDate(match.date ? match.date.slice(0, 10) : '')
        setTime(match.time || '')
        setMaxPlayers(match.maxPlayers || 10)
        setSkillLevel(match.skillLevel || '')
        setNotes(match.description || '')

      } catch (err) {
        setErrors({ server: 'Could not load match details.' })
      } finally {
        setFetching(false)
      }
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Go back to the organizer match page after saving
      navigate('/organizer-match/' + id)
  ...
  if (fetching) {
    return (
      <div className="create-match-page">
        ...
        <div style={{ padding: 60, textAlign: 'center', color: '#6B7280' }}>Loading match details...</div>
      </div>
    )
  }
    ...
    <div className="create-match-page">
      ...
      <div className="create-match-layout">
        ...
        <div className="create-match-form-panel">
          ...
          <h1 className="create-match-heading">Edit Match</h1>
          <p className="create-match-subheading">Update the details for your game</p>
